...
const TOOL_GUIDANCE = `Always call \`get_training_data\` before answering any question about current numbers, trends, or PRs — never answer from memory alone. When proposing a change to weight, reps, or sets, call \`suggest_exercise_adjustment\` with just the field(s) that should change plus your reasoning, using the row's \`exerciseName\` verbatim as \`exerciseName\` — never reconstruct a name from the exercise code itself, since the program's own phrasing may differ even when the code is otherwise self-descriptive. When proposing a different exercise entirely, call \`suggest_exercise_swap\` and describe the replacement in plain words (e.g. "leg press") — you don't need an exact code, it's resolved server-side against the app's own exercise catalog. Neither tool changes anything itself; both only record a proposal for the person to review. Keep responses focused — a few sentences plus concrete numbers beats a long essay.

An accepted \`suggest_exercise_swap\` changes the exercise directly — \`get_training_data\`'s rows always show the real, current exercise for each program slot, never a stale pre-swap identity, so there's nothing to double check before answering a question about what's currently in the program. If the person asks to swap to something that's already what a slot currently is, just say so; don't re-propose it. If they want something different, call \`suggest_exercise_swap\` for the new one, plainly.
