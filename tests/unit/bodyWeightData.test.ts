...
// Supports both the select().eq().gte().order() chain readCachedWeightDays
// uses AND the upsert() call the persistence side effect uses.
function mockWeightDb(cachedRows: unknown[] = []) {
  const upsert = vi.fn().mockResolvedValue({ error: null });
  const chain = {
    select: () => chain,
    eq: () => chain,
...
describe("getBodyWeightData — happy path", () => {
  it("converts grams to lb (rounded to 0.1) and returns ascending days with the last as latest", async () => {
    vi.mocked(rollUpDataPoints).mockResolvedValue({
      rollupDataPoints: [
        rollupPoint("2026-08-21", 81200),
        rollupPoint("2026-08-20", 80800),
      ],
    });
    mockUpsert();
  ...
  it("skips a day with no weight field rather than plotting it as 0 lb", async () => {
    vi.mocked(rollUpDataPoints).mockResolvedValue({
      rollupDataPoints: [
        rollupPoint("2026-08-20", 80000),
        rollupPoint("2026-08-21", undefined),
      ],
    });
    mockUpsert();
  ...
  it("passes the requested days through as the fetch window, defaulting to 6", async () => {
    ...
    const spanDays = (range: {
      ...
      const toDate = (c: { year: number; month: number; day: number }) =>
        new Date(c.year, c.month - 1, c.day);
      return Math.round(
        (toDate(range.end.date).getTime() -
          toDate(range.start.date).getTime()) /
          86400000,
      );
    };
  ...
  it("upserts the fetched days into google_health_weight, best-effort", async () => {
    vi.mocked(rollUpDataPoints).mockResolvedValue({
      rollupDataPoints: [rollupPoint("2026-08-20", 80800)],
    });
...
describe("getBodyWeightData — error handling", () => {
  it("returns status: error when the API call throws", async () => {
    vi.mocked(rollUpDataPoints).mockRejectedValue(
      new Error("Google Health API 500: upstream failure"),
    );
    mockUpsert();
...
// A row already cached for today is exactly as fresh as fetching again
// right now would be — weight is a daily aggregate, not a live reading.
// These tests exist to prove the skip actually happens, not just that it
// typechecks: every describe block above never populates the cache mock,
// so their cache-read always fails silently and falls through to the live
// fetch, which would hide a broken skip condition.
describe("getBodyWeightData — cache hit skips the live fetch", () => {
  it("returns cached rows directly, without calling rollUpDataPoints, when today is already cached", async () => {
    const today = new Date().toISOString().slice(0, 10);
    mockWeightDb([{ d: today, weight_lb: 178.1 }]);

    const result = await getBodyWeightData("user-1");

    expect(rollUpDataPoints).not.toHaveBeenCalled();
    expect(result).toEqual({ status: "ok", days: [{ date: today, weightLb: 178.1 }], latest: { date: today, weightLb: 178.1 } });
  });

  it("falls through to the live fetch when the cache has data but nothing for today yet", async () => {
    mockWeightDb([{ d: "2020-01-01", weight_lb: 178.1 }]);
    vi.mocked(rollUpDataPoints).mockResolvedValue({ rollupDataPoints: [rollupPoint("2026-08-20", 80800)] });

    const result = await getBodyWeightData("user-1");

    expect(rollUpDataPoints).toHaveBeenCalled();
    expect(result).toMatchObject({ status: "ok", days: [{ date: "2026-08-20" }] });
  });
