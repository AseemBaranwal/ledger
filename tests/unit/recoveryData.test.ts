...
// Supports both the select().eq().gte().order() chain readCachedRecoveryDays
// uses AND the upsert() call the persistence side effect uses, off the same
// from() return value — a real call site exercises both in one request when
// the live-fetch path runs (cache-check first, upsert at the end).
function mockRecoveryDb(cachedRows: unknown[] = []) {
  const upsert = vi.fn().mockResolvedValue({ error: null });
  const chain = {
    select: () => chain,
    eq: () => chain,
    gte: () => chain,
    order: () => Promise.resolve({ data: cachedRows, error: null }),
  };
  vi.mocked(supabaseAdmin).mockReturnValue({ from: () => ({ ...chain, upsert }) } as any);
  return { upsert };
}
...
vi.mock("../../api/_lib/googleHealth", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("../../api/_lib/googleHealth")>();
  return {
    ...actual,
    getValidAccessToken: vi.fn(),
    googleHealthRequest: vi.fn(),
  };
});
...
function mockDataPoints(byType: {
  ...
  vi.mocked(googleHealthRequest).mockImplementation(
  ...
  return {
    sleep: {
      interval: {
        ...
        endUtcOffset: opts.endUtcOffset ?? "0s",
      },
      type: opts.type,
      metadata:
        opts.mainSleep != null ? { mainSleep: opts.mainSleep } : undefined,
      summary: {
...
describe("getRecoveryData — happy path", () => {
  it("flattens all three data types into one row per date, ascending", async () => {
    mockDataPoints({
      restingHeartRate: [
        rhrPoint("2026-08-20", "54"),
        rhrPoint("2026-08-21", "59"),
      ],
  ...
  it("parses int64 wire fields serialized as strings", async () => {
    mockDataPoints({
      ...
      sleep: [
        sleepPoint({ endTime: "2026-08-20T06:00:00Z", minutesAsleep: "415" }),
      ],
    });
    ...
    expect(result).toMatchObject({
      days: [{ restingHeartRate: 61, sleepMinutes: 415 }],
    });
  });
  ...
  it("calls dataPoints.list per data type, in parallel, with a date-range filter", async () => {
    ...
    expect(paths[1]).toContain(
      "users/me/dataTypes/daily-heart-rate-variability/dataPoints?",
    );
  ...
  it("uses the median, not the mean, so a single outlier night does not move the baseline", async () => {
    mockDataPoints({
      restingHeartRate: [
        rhrPoint("2026-08-18", "55"),
        rhrPoint("2026-08-19", "56"),
        rhrPoint("2026-08-20", "120"),
      ],
    });
  ...
  it("totals multiple sleep sessions per day (a nap plus a night)", async () => {
    ...
    expect(result).toMatchObject({
      days: [{ date: "2026-08-20", sleepMinutes: 425 }],
    });
  });
...
describe("getRecoveryData — sleep quality index", () => {
  it("computes a score from duration + efficiency + restoration for a real STAGES night", async () => {
    mockDataPoints({
      sleep: [goodNight("2026-08-20", "2026-08-20T06:00:00Z")],
    });
  ...
  it("ignores a nap for efficiency/type fields, using only the flagged main sleep", async () => {
    ...
    expect(result).toMatchObject({
      days: [{ sleepMinutes: 458, sleepQualityIndex: 92 }],
    });
  });
});
...
describe("getRecoveryData — deltas, flags, readiness", () => {
  it("flags an elevated resting heart rate, a dropped HRV, and short sleep as compromised", async () => {
    ...
    expect(
      flags.some((f) => f.includes("Resting heart rate is 13 bpm above")),
    ).toBe(true);
  ...
  it("flags two or more consecutive reduced-sleep nights", async () => {
    ...
    const flags = (result as { flags: string[] }).flags;
    expect(
      flags.some((f) => f.includes("consecutive nights of reduced sleep")),
    ).toBe(true);
  });
...
describe("getRecoveryData — partial failure", () => {
  it("returns the data types that succeeded when one of them errors", async () => {
    mockDataPoints({
      ...
      sleep: [
        sleepPoint({ endTime: "2026-08-20T06:00:00Z", minutesAsleep: "420" }),
      ],
    });
  ...
  it("reports an error only when all three data types fail", async () => {
    ...
    expect(await getRecoveryData("user-1")).toEqual({
      status: "error",
      error: "Google Health API 500: boom",
    });
  });
});

// This is a side-effect CACHE, not the Coach's source of truth — the Coach
// still always gets the live fetch above, unchanged. google_health_recovery
// exists purely so the Trends tab's Recovery domain and the Sheet export
// have real rows to read without paying for a live Google round-trip on
// every read (issue #74's follow-up).
describe("getRecoveryData — persistence side effect", () => {
  it("upserts every returned day into google_health_recovery", async () => {
    mockDataPoints({
      restingHeartRate: [rhrPoint("2026-08-20", "54")],
      hrv: [hrvPoint("2026-08-20", 62.34)],
      sleep: [
        sleepPoint({ endTime: "2026-08-20T06:00:00Z", minutesAsleep: "431" }),
      ],
    });
  ...
  it("does not let a persistence failure break the returned result", async () => {
    mockDataPoints({ restingHeartRate: [rhrPoint("2026-08-20", "54")] });
  ...
  it("does not upsert anything when there are no days at all", async () => {
    mockDataPoints({});
    const upsert = mockUpsert();
...
// A row already cached for today is exactly as fresh as a live fetch right
// now would be (RHR/HRV/sleep are finished daily aggregates, not something
// that changes again mid-day) — see readCachedRecoveryDays's own comment.
// These tests exist specifically to prove the skip actually happens, not
// just that it typechecks: the other describe blocks above never populate
// the cache mock, so their cache-read always fails silently and falls
// through to the live fetch, which would hide a broken skip condition.
describe("getRecoveryData — cache hit skips the live fetch", () => {
  it("returns a result computed from cached rows, without calling googleHealthRequest, when today is already cached", async () => {
    const today = new Date().toISOString().slice(0, 10);
    mockRecoveryDb([
      { d: today, resting_heart_rate: 58, hrv_ms: 61.2, sleep_minutes: 410, sleep_quality_index: 72 },
    ]);
    ...
    expect(result).toMatchObject({
      status: "ok",
      days: [{ date: today, restingHeartRate: 58, hrvMs: 61.2, sleepMinutes: 410, sleepQualityIndex: 72 }],
      latest: { date: today, restingHeartRate: 58 },
    });
  });

  it("falls through to the live fetch when the cache has data but nothing for today yet", async () => {
    mockRecoveryDb([{ d: "2020-01-01", resting_heart_rate: 58, hrv_ms: null, sleep_minutes: null, sleep_quality_index: null }])
    mockDataPoints({ restingHeartRate: [rhrPoint("2026-08-20", "55")] })

    const result = await getRecoveryData("user-1");

    expect(googleHealthRequest).toHaveBeenCalled();
    expect(result).toMatchObject({ status: "ok", days: [{ date: "2026-08-20", restingHeartRate: 55 }] });
  });

  it("falls through to the live fetch when the cache read itself fails", async () => {
    vi.mocked(supabaseAdmin).mockReturnValue({
